            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex bg-gray-800 border border-gray-600 rounded-lg p-0.5 text-xs">
                    <button type="button"
                      onClick={() => setYamlMode("editor")}
                      className={`px-3 py-1 rounded-md transition-colors ${
                        yamlMode === "editor" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"
                      }`}>
                      ✏️ Editor
                    </button>
                    <button type="button"
                      ...
                      className={`px-3 py-1 rounded-md transition-colors ${
                        yamlMode === "file" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"
                      }`}>
                      📂 File
                    </button>
                  </div>
                ...
                {yamlMode === "editor" ? (
                  <div className="relative">
                    <textarea
                      value={scenariosYaml}
                      onChange={(e) => setScenariosYaml(e.target.value)}
                      rows={12}
                      spellCheck={false}
                    ...
                    <button
                      type="button"
                      onClick={() => setScenariosYaml(SAMPLE_YAML)}
                      className="absolute top-2 right-2 text-xs text-gray-500 hover:text-gray-300 bg-gray-900 border border-gray-700 rounded px-2 py-0.5"
                    >
                      Load sample
                    </button>
                  </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  POST {`{ question, context }`} → expects {`{ answer }`}. Leave blank for echo mode.
                </p>
              </div>
                  ...
                  <tr key={run.run_id} className={i % 2 === 0 ? "bg-gray-900" : "bg-gray-950"}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => handleCopyShareLink(run.run_id)}
